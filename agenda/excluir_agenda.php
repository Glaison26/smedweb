...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result_delete) {
        // gero log de exclusão
        $usuario = $_SESSION['c_userId'];
        $data_atual = date('Y-m-d');
        $hora_atual = date('H:i:s');
        $descricao = "Exclusão de agenda do médico $medico_nome (id $medico_id)";
        $registro = "Periodo:" . date("d/m/Y", strtotime($_POST['from_date']))  . " a " . date("d/m/Y", strtotime($_POST['to_date']));
        $c_sql_log = "INSERT INTO log_criacao_agenda (data,hora,id_usuario,descricao,registro)
        VALUES ('$data_atual','$hora_atual','$usuario','$descricao','$registro')";
        //echo $c_sql_log;
    ...
    <script>
        ...
        form && form.addEventListener('submit', function(e) {
            ...
            if (!confirmBox.checked) {
                e.preventDefault();
                alert('Confirme que entendeu os riscos.');
                return;
            }
            if (mode === 'between') {
                ...
                if (fromDate.value > toDate.value) {
                    e.preventDefault();
                    alert('Data de início maior que a data fim.');
                    return;
                }
            }
            if (mode === 'before' && !beforeDate.value) {
                e.preventDefault();
                alert('Informe a data.');
                return;
            }
            if (mode === 'lastdays' && (!lastDays.value || lastDays.value < 1)) {
                e.preventDefault();
