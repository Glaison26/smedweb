...
if ($registro['usa_epi'] == 'Sim') {
    $c_historia .= "Qual EPI :" . $registro['quais_epi'] . "\r\n" . "\r\n";
}
// riscos ocupacionais
$c_historia .= 'Riscos Ocupacionais :' . "\r\n" . "\r\n";
// risco fisico
if ($registro['risco_fisico'] == 'S') {
    $c_historia .= 'Físico (Ruído, calor, frio, vibração, radiação) : Sim' . "\r\n";
} else {
    $c_historia .= 'Físico (Ruído, calor, frio, vibração, radiação) : Não' . "\r\n";
}
// risco quimico
if ($registro['risco_quimico'] == 'S') {
    $c_historia .= 'Químico (Poeira, fumos, gases,vapores, produtos químicos) : Sim' . "\r\n";
} else {
    $c_historia .= 'Químico (Poeira, fumos, gases,vapores, produtos químicos) : Não' . "\r\n";
}
// risco biológico
if ($registro['risco_biologico'] == 'S') {
    $c_historia .= 'Biológico (Vírus, bactérias, fungos, parasitas) : Sim' . "\r\n";
} else {
    $c_historia .= 'Biológico (Vírus, bactérias, fungos, parasitas) : Não' . "\r\n";
}
// risco ergonomico
if ($registro['risco_ergonomico'] == 'S') {
    $c_historia .= 'Ergonômico (Postura inadequada, esforço repetitivo, levantamento de peso) : Sim' . "\r\n";
} else {
    $c_historia .= 'Ergonômico (Postura inadequada, esforço repetitivo, levantamento de peso) : Não' . "\r\n";
}
//risco Acidentes
if ($registro['risco_acidentes'] == 'S') {
    $c_historia .= 'Acidentes (Máquinas sem proteção, risco de quedas, eletricidade) : Sim' . "\r\n";
} else {
    $c_historia .= 'Acidentes (Máquinas sem proteção, risco de quedas, eletricidade) : Não' . "\r\n";
}
// Queixa Principal e História da Doença Atual (HDA)
$c_historia .= 'Queixa Principal e História da Doença Atual (HDA)' . "\r\n" . "\r\n";
$c_historia .= 'Motivo da Consulta: ' . $registro['motivo_consulta'] . "\r\n" .
...
if ($registro['habito_tabagismo'] == 'Sim') {
    $c_historia .= 'Cigarros por dia :' . $registro['tabagismo_qtd_dia'] . "\r\n" .
        'Há quanto tempo (anos):' . $registro['tabagismo_tempo'] . "\r\n" . "\r\n";
}
// etilismo
$c_historia .= 'Etilismo :' . $registro['etilismo'] . "\r\n";
if ($registro['etilismo'] == 'Sim') {
    $c_historia .= 'Doses Semanais: ' . $registro['etilismo_frequencia'] . "\r\n" . "\r\n";
}
// atividade fisica
$c_historia .= 'Atividade Física :' . $registro['atividade_fisica'] . "\r\n";
if ($registro['atividade_fisica'] == 'Sim') {
    $c_historia .= 'Quais atividades Físicas :' . $registro['atividade_fisica_qual'] . "\r\n" .
        'Frequência :' . $registro['atividade_fisica_frequencia'] . "\r\n". "\r\n";
}
// Antecedentes Familiares
$c_historia .= "Antecedentes Familiares"."\r\n"."\r\n".
...
$c_historia .= "Interrogatório Sintomatológico (Revisão por Sistemas)". "\r\n". "\r\n".
...
$c_historico .= 'Conduta e Parecer Médico'."\r\n"."\r\n".
...
$c_historia .= 'Parecer Médico: '. $c_parecer."\r\n";
if ($c_restricoes!='R'){
   $c_historia .= 'Restrições: '. $c_restricoes."\r\n"; 
}
// atualiza a historia
...
$conection->close();
/////////////////////// fim da importação para o histórico /////////////////////////////////////
