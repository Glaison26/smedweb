...
try {
    ...
    $relatorio = $_SESSION['relatorio'] ?? null;
     // Verifica se a sessão de orientação está definida
    if (!$relatorio) {
        throw new Exception('Relatório não encontrado na sessão.');
    }
    $html = gerarHTMLOrientacao($relatorio);

    if ($formato === 'pdf') {
        gerarPDF($html, $relatorio);
    } else {
        echo $html;
    }
} catch (Exception $e) {
...
function gerarHTMLOrientacao($relatorio)
{
    include("..\conexao.php");
    ...
    <head>
        ...
        <title>Relatório Médica - " . $paciente . "'</title>
        <style>
            body {
    ...
    <body>
        ...
        <h1 class='titulo'>Relatório Médico</h1>
        ...
        <div class='rodape'>
            <p>Relatório médico gerado em: {$data_atual} </p>
